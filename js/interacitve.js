...
function interactive() {
  $("circle").click(function(event) {
    ...
    if (event.target.__data__.key === "2005") {
      stateMessage = `<p>Missing information for 2005</p>`
    } else {
      if (stateName.includes("")) {
        stateMessage = `<p>Missing information about the state, during ${event.target.__data__.key}</p>
          <p>sers from ${stateName.length - 1} states applied loan from Prosper</p>`
      } else {
        stateMessage = `<p>Users from ${stateName.length} states applied loan from Prosper</p>`
      }
    }

    let message = `<p>During ${event.target.__data__.key}, Total loan amount: \$${event.target.__data__.value.loan.toLocaleString("USD")}</p>`
    ...
    $("div.loan-map>svg>g>path").each((index, path) => {
      if (stateName.includes(path.__data__.properties.STUSPS10)) {
        d3.select(path).style("fill", "#FBB978");
      } else {
...
function mapReport(fixdata) {
  ...
  message = `<h3>During ${d3.timeFormat("%Y %B")(fixdata.value["date"])},
            Total loan amount: \$${fixdata.value["totalloan"].toLocaleString("USD")}.</h3>`;
  if (fixdata.value["state"].includes("") && fixdata.value["state"].length === 1) {
    optionMessage = `<h3>Missing state inforamtion.</h3>`
  } else if (fixdata.value["state"].length > 1) {
    if (fixdata.value["state"].includes("")) {
      optionMessage = `<h3>Missing state infomation</h3>
        <h3>Users from ${fixdata.value["state"].length - 1} states applied loan from Prosper</h3>`
    } else {
      optionMessage = `<h3>Users from  ${fixdata.value["state"].length} states applied loan from Prosper</h3>`
    }
  }
